...
Marginal Demand for Kern County P = 342.9 - 2.286E
E=(P-342.9)/-2.286
Marginal Demand for Mojave County P = 532.56 - 3.804E
E=(P-532.56)/-3.804
Marginal Demand for Antelope County P= 628.76 - 2.858E
E=(P-628.76)/-2.858
Marginal Demand for Ventura County P= 435.12 - 1.776E
E=(P-435.12)/-1.776

#sum E curves 
E = (P-342.9)/-2.286 + (P-532.56)/-3.804 + (P-628.76)/-2.858 + (P-435.12)/-1.776
E = 755.0-1.6133P
#solve for P
P=(E-755)/-1.6133
P=467.98-0.61985E
...
```{r}
#creating aggregate demand curve 
agg_vals <- 0:245
agg_MC = 467.98-0.61985*(agg_vals)
df_agg <- data.frame(Reduction = agg_vals,
                   predicted_mc = agg_MC, 
                   water_demand = rev(agg_vals))

plot(agg_vals, agg_MC)
```
...
```
2. Antelope irrigation district are willing to pay the most for the first acre-foot of water. 
...
Antelope abatement cost is: `r antelop_cost`
Ventura abatement cost is: `r venturea_cost`
...
Antelope abatement cost with tax is: `r antelop_cost_b`
Ventura abatement cost with tax is: `r venturea_cost_b`
...
calc_revenue <- function(baseline,quantity_abate){
  revenue <- (158.09 * baseline-quantity_abate)
  return(revenue)
}

revnue_kern <- calc_revenue(baseline=140,quantity_abate=69.14)
revnue_mojave <- calc_revenue(baseline=150,quantity_abate=41.56)
revnue_antelope <- calc_revenue(baseline=220,quantity_abate=55.31)
revnue_ventura <- calc_revenue(baseline=240,quantity_abate=89.01)
...
tax = 158.09
total_tax_revenue2 = total_quantity_used*tax
total_tax_revenue2
```
Total government revenue generated from tax: $`r total_tax_revnue`
...
Antelope abatement cost is: `r antelop_cost_4`
Ventura abatement cost is: `r venturea_cost_4`
...
ventura_cost_b_4 <- calc_cost(ventura_lm, quantity_abate = 158.82)
...
revnue_ventura_4 <- calc_revenue(baseline=245,quantity_abate=158.82)
...
ventura_after_trade_4 <-estimated_cost(ventura_lm,158.82)
...
additional_benefit_ventura_4 <- additional_benefit(ventura_before_trade_4, ventura_after_trade_4,146.76,158.82)
