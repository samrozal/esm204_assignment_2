...
- This is done by first converting all of my marginal cost equations to solve for the Q variable and then I will be horizontally summing the Q value. 
...
Now, we need to calculate the Q value (quantity abated) associated with this P* for each district/county's marginal abatement cost curve. To do this, we plug in P = 158.09 into each of the marginal cost curves. 
...
#write a function to calculate tax revenue
calc_revenue <- function(baseline,quantity_abate){
  revenue <- (158.09 * (baseline-quantity_abate))
  return(revenue)
}

revnue_kern <- calc_revenue(baseline=150,quantity_abate=69.14)
revnue_mojave <- calc_revenue(baseline=140,quantity_abate=41.56)
revnue_antelope <- calc_revenue(baseline=220,quantity_abate=55.31)
revnue_ventura <- calc_revenue(baseline=245,quantity_abate=89.01)
